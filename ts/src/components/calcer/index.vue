<!--  -->
<template>
<Page :title="select"  :titleValue="calcerType" @titleChange="titleChange" borderLine padding >
        <div class="calcer">
            <div style="paddingTop: 60px" v-if="!isMobile">
                <select class="tcolor f14" v-model="calcerType">
                    <option :value="index" v-for="(se,index) in select" :key="index">{{$t(`m.${se}`)}}</option>
                </select>
            </div>
...
export default {
  ...
  created() {
      this.calcerType = !this.isMobile ? this.panelData.type : this.$route.query.type;
  },
  mounted() {},
  methods: {
      changeCalcer(n) {
