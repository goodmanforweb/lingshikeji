<template>
    <div :class="`${isMobile ? 'page re' : ''}`" >
        <template v-if="isMobile">
            ...
            >
                ...
                <span>
                    ...
                    <span class="page-title tcolor f20 fw fuper" v-else-if="title">{{title}}</span>
        ...
        <template v-else>
            <slot v-if="!pcIsSlot"/>
            <div v-else class="right-panel" :class="pcPanelClassName">
                <span class="n-btn re-close" v-if="pcPanelFullScreen" @click="$router.push(routerBack)">{{$t('m.close')}}</span>
                <div class="rp-aside " @click.stop :class="{fullScreen: pcPanelFullScreen}">
                    <slot />
                </div>
            </div>
        </template>
...
export default {
    props: [
        ...
        'pcPanelClassName',
        'pcPanelFullScreen',
    'float','fixed', 'title','titleValue','subtitle','hasBackBtn','styleBackIcon', 'padding', 'borderLine', 'routerBack','rightBtn'],
    data() {
        return {
            titleselect:0,
        }
