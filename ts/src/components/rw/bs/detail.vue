...
<div class="dot-detail">
    ...
    <div class="sb mb16" :class="{flex: !isMobile}">
      <div class="bn-cube center block" :class="{'flex sb vc mb16':isMobile}">
        <div class="gcolor">{{$t('m.trjjzall')}}</div>
        <span class="tcolor f24">{{item.yield_rate}}%</span>
      </div>
      <div class="bn-cube center block" :class="{'flex sb vc':isMobile}">
        <div class="gcolor">{{$t('m.sdybjztz')}}</div>
        <span class="tcolor f24">{{suoding}}%</span>
      </div>
    </div>
    <div class="sb" :class="{flex: !isMobile}">
      <div class="bn-cube center block" :class="{'flex sb vc mb16':isMobile}">
        <div class="gcolor">{{$t('m.ybjmzll')}}</div>
        <span class="tcolor f24"> {{lilv[0]}}% ~ {{lilv[1]}}%</span>
      </div>
      <div class="bn-cube center block" :class="{'flex sb vc':isMobile}">
        <div class="gcolor">{{$t('m.hyzqi')}}</div>
        <span class="tcolor f24">{{item.experience == 0 ? $t('m.cqyx') : item.experience + $t('m.day')}}</span>
      </div>
    </div>
...
export default {
    ...
    computed: {
      ...mapGetters(['signHYDetail','useraccount']),
        ...
        suoding() {
          return Big(100).minus(this.item.yield_rate).toNumber();
        },
        lilv() {
          if(this.item.interest_rate) {
              return this.item.interest_rate.split(',')
          }
          return [0,0]
      }
    },
