<!--  -->
<template>
<Page :title="$t('m.keybuyjl')" borderLine padding routerBack="bdash">

<div class="tx-record panel-width">
    
    <div class="f24 tcolor fw mb16" v-if="!isMobile">{{$t('m.keybuyjl')}}</div>
  ...
  methods: {
      ...
      fetch() {
          if(this.loader || this.finish || (this.hasTotal && this.page*this.size >= this.total)) return Promise.resolve();
            this.page++;
            return this.getBNDepositRecord()
      },
      getBNDepositRecord() {
          this.loader = true;
          return request.bnDepositLists({
              params: {
                  order:'created_time',
                  desc: true,
                  project: this.id,
