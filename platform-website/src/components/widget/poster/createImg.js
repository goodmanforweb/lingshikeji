let template = require("../../../assets/images/poster.png");
let templateen = require("../../../assets/images/posteren.png");
...
class bigman {
  ...
  addInfo(text, link, callback) {
    ...
    this.ctx.fillText(text, x, 540);

    return this.createQrcode(link).then(url => {
      this.getImg(url).then(qr => {
        this.qrcodeimg = qr;
        this.ctx.drawImage(qr, 160, 325, 186, 186);
        callback && callback();
      });
    })
  }
